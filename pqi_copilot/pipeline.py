...
from pqi_copilot.common import file_sha256, normalize_token, stable_hash_obj, write_json, write_yaml
...
def propose_run(input_dir: Path, ig_source: Path | None = None) -> dict[str, Any]:
    ...
    mapping_path = write_run_artifact(run_id, "mapping_proposals.json", proposals)
    relationship_path = write_run_artifact(run_id, "relationship_proposals.json", relationships)
    ...
    manifest = run_manifest(
        ...
        mapping_proposal_hash=file_sha256(mapping_path),
    )
    manifest_path = write_run_artifact(run_id, "manifest.json", manifest)
    ...
    return {
        ...
        "mapping_proposal_hash": file_sha256(mapping_path),
        "relationship_proposal_hash": file_sha256(relationship_path),
    }
